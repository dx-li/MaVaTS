# Matrixwise Huber factors: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#fit_huber_factor)
...
Paper attribution: [he2024huber](../citations.md#he2024huber). This is a new teaching application,
...
```bash
python -m examples.air_quality --methods huber --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "huber")
dates, values, counts = load()
x, observed, preprocessing = preprocess(values, counts, train=273, tensor=False)
from mavats import fit_huber_factor
result = fit_huber_factor(x[:273], ranks=(1, 1))
```
...
have no substituted estimate. [Full diagnostics](huber.json) include
...
Computed reconstruction_discrepancy: **0.2593** over 1042 observed target cells. 

![Matrixwise Huber factors: computed output and diagnostics; see adjacent interpretation and JSON data](huber.png)
...
[Numerical plot/score arrays](huber.npz) and [run provenance](provenance.json)
