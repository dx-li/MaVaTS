# Two-lag MAR: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods mar-multilag --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "mar-multilag")
...
result = fit_mar(x[:273], order=2, fit_intercept=True, max_iter=500)
```
...
have no substituted estimate. [Full diagnostics](mar-multilag.json) include
...
Computed forecast_mse: **1.3290** over 1042 observed target cells. Previous-day MSE: 1.8212; fixed-training-mean MSE: 1.6649. Lower is better on this split only; no uncertainty interval is estimated.

![Two-lag MAR: computed output and diagnostics; see adjacent interpretation and JSON data](mar-multilag.png)
...
[Numerical plot/score arrays](mar-multilag.npz) and [run provenance](provenance.json)
