...
```bash
python -m examples.air_quality --methods alpha-pca-0 --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "alpha-pca-0")
...
have no substituted estimate. [Full diagnostics](alpha-pca-0.json) include
...
![Alpha-PCA: alpha=-1: computed output and diagnostics; see adjacent interpretation and JSON data](alpha-pca-0.png)
...
[Numerical plot/score arrays](alpha-pca-0.npz) and [run provenance](provenance.json)
