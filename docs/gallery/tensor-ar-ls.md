# Tensor AR: LS: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods tensor-ar-ls --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "tensor-ar-ls")
...
result = fit_tensor_ar(x[:273], method='ls', terms=(1,), max_iter=200)
```
...
have no substituted estimate. [Full diagnostics](tensor-ar-ls.json) include
...
Computed forecast_mse: **1.1558** over 2079 observed target cells. Previous-day MSE: 2.3205; fixed-training-mean MSE: 1.7401. Lower is better on this split only; no uncertainty interval is estimated.

![Tensor AR: LS: computed output and diagnostics; see adjacent interpretation and JSON data](tensor-ar-ls.png)
...
[Numerical plot/score arrays](tensor-ar-ls.npz) and [run provenance](provenance.json)
