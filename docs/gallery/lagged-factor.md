# Lagged matrix factors: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#fit_lagged_factor)
...
Paper attribution: [wang2019matrixfactor](../citations.md#wang2019matrixfactor). This is a new teaching application,
...
```bash
python -m examples.air_quality --methods lagged-factor --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "lagged-factor")
dates, values, counts = load()
x, observed, preprocessing = preprocess(values, counts, train=273, tensor=False)
from mavats import fit_lagged_factor
result = fit_lagged_factor(x[:273], ranks=(1, 1))
```
...
have no substituted estimate. [Full diagnostics](lagged-factor.json) include
...
Computed reconstruction_discrepancy: **0.2538** over 1042 observed target cells. 

![Lagged matrix factors: computed output and diagnostics; see adjacent interpretation and JSON data](lagged-factor.png)
...
[Numerical plot/score arrays](lagged-factor.npz) and [run provenance](provenance.json)
