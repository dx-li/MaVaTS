# Alpha-PCA: alpha=0: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods alpha-pca-1 --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "alpha-pca-1")
...
result = fit_alpha_pca(x[:273], ranks=(1, 1), alpha=0)
```
...
have no substituted estimate. [Full diagnostics](alpha-pca-1.json) include
...
![Alpha-PCA: alpha=0: computed output and diagnostics; see adjacent interpretation and JSON data](alpha-pca-1.png)
...
[Numerical plot/score arrays](alpha-pca-1.npz) and [run provenance](provenance.json)
