# Two-way dynamic factors: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#fit_two_way_dynamic)
...
Paper attribution: [yuan2023dynamic](../citations.md#yuan2023dynamic). This is a new teaching application,
...
```bash
python -m examples.air_quality --methods two-way-dynamic --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "two-way-dynamic")
dates, values, counts = load()
x, observed, preprocessing = preprocess(values, counts, train=273, tensor=False)
from mavats import fit_two_way_dynamic
result = fit_two_way_dynamic(x[:273], ranks=(1, 1), max_iter=200)
```
...
have no substituted estimate. [Full diagnostics](two-way-dynamic.json) include
...
Computed forecast_mse: **1.3418** over 1042 observed target cells. Previous-day MSE: 1.8212; fixed-training-mean MSE: 1.6649. Lower is better on this split only; no uncertainty interval is estimated.

![Two-way dynamic factors: computed output and diagnostics; see adjacent interpretation and JSON data](two-way-dynamic.png)
...
[Numerical plot/score arrays](two-way-dynamic.npz) and [run provenance](provenance.json)
