# MAR marginal intervals: PROJECTION: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods inference-projection --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "inference-projection")
...
result = mar_inference(x[:273], method='projection')
```
...
have no substituted estimate. [Full diagnostics](inference-projection.json) include
...
![MAR marginal intervals: PROJECTION: computed output and diagnostics; see adjacent interpretation and JSON data](inference-projection.png)
...
[Numerical plot/score arrays](inference-projection.npz) and [run provenance](provenance.json)
