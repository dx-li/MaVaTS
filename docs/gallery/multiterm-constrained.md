# Multi-term constrained factors: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#fit_multiterm_constrained_factor)

## Question and applicability

Can a small shared loading space summarize new station-by-pollutant observations?

Scores use each held-out observation itself: this is reconstruction, not forecasting. The unobserved clean signal is unknown, so discrepancy is not denoising accuracy. Two disjoint pollutant spans separate particulate and NO2 components algebraically; this does not attribute emissions to sources.

Paper attribution: [chen2020constrained](../citations.md#chen2020constrained). This is a new teaching application,
...
```bash
python -m examples.air_quality --methods multiterm-constrained --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "multiterm-constrained")
...
have no substituted estimate. [Full diagnostics](multiterm-constrained.json) include
...
Computed reconstruction_discrepancy: **0.1579** over 1042 observed target cells. 

![Multi-term constrained factors: computed output and diagnostics; see adjacent interpretation and JSON data](multiterm-constrained.png)
...
[Numerical plot/score arrays](multiterm-constrained.npz) and [run provenance](provenance.json)
