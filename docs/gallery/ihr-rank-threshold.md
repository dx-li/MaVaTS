# IHR rank selection: threshold: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods ihr-rank-threshold --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "ihr-rank-threshold")
...
result = select_ihr_ranks(x[:273], max_ranks=(2, 2), method='threshold')
```
...
have no substituted estimate. [Full diagnostics](ihr-rank-threshold.json) include
...
![IHR rank selection: threshold: computed output and diagnostics; see adjacent interpretation and JSON data](ihr-rank-threshold.png)
...
[Numerical plot/score arrays](ihr-rank-threshold.npz) and [run provenance](provenance.json)
