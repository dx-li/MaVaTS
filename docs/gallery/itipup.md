# ITIPUP tensor factors: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods itipup --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "itipup")
...
result = fit_tensor_factor(x[:273], ranks=(1, 1, 1), method='tipup', iterative=True)
```
...
have no substituted estimate. [Full diagnostics](itipup.json) include
...
![ITIPUP tensor factors: computed output and diagnostics; see adjacent interpretation and JSON data](itipup.png)
...
[Numerical plot/score arrays](itipup.npz) and [run provenance](provenance.json)
