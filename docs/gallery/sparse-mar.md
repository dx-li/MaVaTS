# Sparse MAR: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#fit_sparse_mar)
...
Paper attribution: [celani2024sparse](../citations.md#celani2024sparse). This is a new teaching application,
...
```bash
python -m examples.air_quality --methods sparse-mar --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "sparse-mar")
dates, values, counts = load()
x, observed, preprocessing = preprocess(values, counts, train=273, tensor=False)
from mavats import fit_sparse_mar
result = fit_sparse_mar(x[:273], max_iter=500)
```
...
have no substituted estimate. [Full diagnostics](sparse-mar.json) include
...
Computed forecast_mse: **1.3214** over 1042 observed target cells. Previous-day MSE: 1.8212; fixed-training-mean MSE: 1.6649. Lower is better on this split only; no uncertainty interval is estimated.

![Sparse MAR: computed output and diagnostics; see adjacent interpretation and JSON data](sparse-mar.png)
...
[Numerical plot/score arrays](sparse-mar.npz) and [run provenance](provenance.json)
