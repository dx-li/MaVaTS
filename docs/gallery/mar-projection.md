# MAR: PROJECTION: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#fit_mar)
...
Paper attribution: [chen2021mar](../citations.md#chen2021mar), [xiao2022rrmar](../citations.md#xiao2022rrmar). This is a new teaching application,
...
```bash
python -m examples.air_quality --methods mar-projection --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "mar-projection")
...
result = fit_mar(x[:273], method='projection', fit_intercept=True, max_iter=500)
```
...
have no substituted estimate. [Full diagnostics](mar-projection.json) include
...
Computed forecast_mse: **1.3545** over 1042 observed target cells. Previous-day MSE: 1.8212; fixed-training-mean MSE: 1.6649. Lower is better on this split only; no uncertainty interval is estimated.

![MAR: PROJECTION: computed output and diagnostics; see adjacent interpretation and JSON data](mar-projection.png)
...
[Numerical plot/score arrays](mar-projection.npz) and [run provenance](provenance.json)
