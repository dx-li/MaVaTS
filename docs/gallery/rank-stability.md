# Rank stability path: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#tensor_rank_stability)
...
```bash
python -m examples.air_quality --methods rank-stability --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "rank-stability")
...
have no substituted estimate. [Full diagnostics](rank-stability.json) include
...
![Rank stability path: computed output and diagnostics; see adjacent interpretation and JSON data](rank-stability.png)
...
[Numerical plot/score arrays](rank-stability.npz) and [run provenance](provenance.json)
