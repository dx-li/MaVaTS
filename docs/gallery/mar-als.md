# MAR: ALS: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods mar-als --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "mar-als")
...
result = fit_mar(x[:273], method='als', fit_intercept=True, max_iter=500)
```
...
have no substituted estimate. [Full diagnostics](mar-als.json) include
...
Computed forecast_mse: **1.3077** over 1042 observed target cells. Previous-day MSE: 1.8212; fixed-training-mean MSE: 1.6649. Lower is better on this split only; no uncertainty interval is estimated.

![MAR: ALS: computed output and diagnostics; see adjacent interpretation and JSON data](mar-als.png)
...
[Numerical plot/score arrays](mar-als.npz) and [run provenance](provenance.json)
