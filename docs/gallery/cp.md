# CP matrix factors: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#fit_cp_factor)

## Question and applicability

Can a small shared loading space summarize new station-by-pollutant observations?

Scores use each held-out observation itself: this is reconstruction, not forecasting. The unobserved clean signal is unknown, so discrepancy is not denoising accuracy. Rank one is predeclared. CP identification guards can legitimately reject a dataset; do not weaken them to obtain a picture.

Paper attribution: [chang2023cp](../citations.md#chang2023cp). This is a new teaching application,
...
```bash
python -m examples.air_quality --methods cp --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "cp")
dates, values, counts = load()
x, observed, preprocessing = preprocess(values, counts, train=273, tensor=False)
from mavats import fit_cp_factor
result = fit_cp_factor(x[:273], rank=1)
```
...
have no substituted estimate. [Full diagnostics](cp.json) include
...
Computed reconstruction_discrepancy: **0.3118** over 1042 observed target cells. 

![CP matrix factors: computed output and diagnostics; see adjacent interpretation and JSON data](cp.png)
...
[Numerical plot/score arrays](cp.npz) and [run provenance](provenance.json)
