# Matrix ARMA: LS: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods marma-ls --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "marma-ls")
...
result = fit_marma(x[:273], method='ls', n_starts=1, max_iter=500)
```
...
have no substituted estimate. [Full diagnostics](marma-ls.json) include
...
Computed forecast_mse: **1.6666** over 1042 observed target cells. Previous-day MSE: 1.8212; fixed-training-mean MSE: 1.6649. Lower is better on this split only; no uncertainty interval is estimated.

![Matrix ARMA: LS: computed output and diagnostics; see adjacent interpretation and JSON data](marma-ls.png)
...
[Numerical plot/score arrays](marma-ls.npz) and [run provenance](provenance.json)
