# MAR: MLE: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods mar-mle --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "mar-mle")
...
result = fit_mar(x[:273], method='mle', fit_intercept=True, max_iter=500)
```
...
have no substituted estimate. [Full diagnostics](mar-mle.json) include
...
Computed forecast_mse: **1.3189** over 1042 observed target cells. Previous-day MSE: 1.8212; fixed-training-mean MSE: 1.6649. Lower is better on this split only; no uncertainty interval is estimated.

![MAR: MLE: computed output and diagnostics; see adjacent interpretation and JSON data](mar-mle.png)
...
[Numerical plot/score arrays](mar-mle.npz) and [run provenance](provenance.json)
