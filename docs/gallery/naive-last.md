# Naive baseline: last: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#fit_naive)
...
Paper attribution: [hyndman2006accuracy](../citations.md#hyndman2006accuracy). This is a new teaching application,
...
```bash
python -m examples.air_quality --methods naive-last --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "naive-last")
dates, values, counts = load()
x, observed, preprocessing = preprocess(values, counts, train=273, tensor=False)
from mavats import fit_naive
result = fit_naive(x[:273], strategy='last')
```
...
have no substituted estimate. [Full diagnostics](naive-last.json) include
...
Computed forecast_mse: **1.8212** over 1042 observed target cells. Previous-day MSE: 1.8212; fixed-training-mean MSE: 1.6649. Lower is better on this split only; no uncertainty interval is estimated.

![Naive baseline: last: computed output and diagnostics; see adjacent interpretation and JSON data](naive-last.png)
...
[Numerical plot/score arrays](naive-last.npz) and [run provenance](provenance.json)
