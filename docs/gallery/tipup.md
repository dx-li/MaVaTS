# TIPUP tensor factors: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods tipup --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "tipup")
...
result = fit_tensor_factor(x[:273], ranks=(1, 1, 1), method='tipup', iterative=False)
```
...
have no substituted estimate. [Full diagnostics](tipup.json) include
...
Computed reconstruction_discrepancy: **0.5976** over 2079 observed target cells. 

![TIPUP tensor factors: computed output and diagnostics; see adjacent interpretation and JSON data](tipup.png)
...
[Numerical plot/score arrays](tipup.npz) and [run provenance](provenance.json)
