# MAR marginal intervals: ALS: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods inference-als --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "inference-als")
...
result = mar_inference(x[:273], method='als')
```
...
have no substituted estimate. [Full diagnostics](inference-als.json) include
...
![MAR marginal intervals: ALS: computed output and diagnostics; see adjacent interpretation and JSON data](inference-als.png)
...
[Numerical plot/score arrays](inference-als.npz) and [run provenance](provenance.json)
