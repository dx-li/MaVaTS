# Cointegrated MAR: MLE: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods cmar-mle --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "cmar-mle")
...
result = fit_cmar(x[:273], ranks=(1, 1), method='mle', n_starts=1, max_iter=500)
```
...
have no substituted estimate. [Full diagnostics](cmar-mle.json) include
...
Computed forecast_mse: **1.4524** over 1042 observed target cells. Previous-day MSE: 1.8212; fixed-training-mean MSE: 1.6649. Lower is better on this split only; no uncertainty interval is estimated.

![Cointegrated MAR: MLE: computed output and diagnostics; see adjacent interpretation and JSON data](cmar-mle.png)
...
[Numerical plot/score arrays](cmar-mle.npz) and [run provenance](provenance.json)
