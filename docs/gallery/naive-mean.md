# Naive baseline: mean: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods naive-mean --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "naive-mean")
...
result = fit_naive(x[:273], strategy='mean')
```
...
have no substituted estimate. [Full diagnostics](naive-mean.json) include
...
Computed forecast_mse: **1.6649** over 1042 observed target cells. Previous-day MSE: 1.8212; fixed-training-mean MSE: 1.6649. Lower is better on this split only; no uncertainty interval is estimated.

![Naive baseline: mean: computed output and diagnostics; see adjacent interpretation and JSON data](naive-mean.png)
...
[Numerical plot/score arrays](naive-mean.npz) and [run provenance](provenance.json)
