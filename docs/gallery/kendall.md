# Matrix Kendall factors: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#fit_matrix_kendall)
...
Paper attribution: [he2025kendall](../citations.md#he2025kendall). This is a new teaching application,
...
```bash
python -m examples.air_quality --methods kendall --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "kendall")
dates, values, counts = load()
x, observed, preprocessing = preprocess(values, counts, train=273, tensor=False)
from mavats import fit_matrix_kendall
result = fit_matrix_kendall(x[:273], ranks=(1, 1))
```
...
have no substituted estimate. [Full diagnostics](kendall.json) include
...
Computed reconstruction_discrepancy: **0.2522** over 1042 observed target cells. 

![Matrix Kendall factors: computed output and diagnostics; see adjacent interpretation and JSON data](kendall.png)
...
[Numerical plot/score arrays](kendall.npz) and [run provenance](provenance.json)
