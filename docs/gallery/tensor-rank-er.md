# Tensor rank selection: ER: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#select_tensor_rank)
...
Paper attribution: [han2022rank](../citations.md#han2022rank). This is a new teaching application,
...
```bash
python -m examples.air_quality --methods tensor-rank-er --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "tensor-rank-er")
dates, values, counts = load()
x, observed, preprocessing = preprocess(values, counts, train=273, tensor=False)
from mavats import select_tensor_rank
result = select_tensor_rank(x[:273], max_ranks=(2, 2), criterion='er', penalty=2)
```
...
have no substituted estimate. [Full diagnostics](tensor-rank-er.json) include
...
![Tensor rank selection: ER: computed output and diagnostics; see adjacent interpretation and JSON data](tensor-rank-er.png)
...
[Numerical plot/score arrays](tensor-rank-er.npz) and [run provenance](provenance.json)
