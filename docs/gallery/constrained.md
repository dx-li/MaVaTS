# Constrained factors: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#fit_constrained_factor)

## Question and applicability

Can a small shared loading space summarize new station-by-pollutant observations?

Scores use each held-out observation itself: this is reconstruction, not forecasting. The unobserved clean signal is unknown, so discrepancy is not denoising accuracy. Equal station loadings and a shared particulate loading are transparent illustrative restrictions, not externally established atmospheric science.
...
```bash
python -m examples.air_quality --methods constrained --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "constrained")
...
have no substituted estimate. [Full diagnostics](constrained.json) include
...
Computed reconstruction_discrepancy: **0.4806** over 1042 observed target cells. 

![Constrained factors: computed output and diagnostics; see adjacent interpretation and JSON data](constrained.png)
...
[Numerical plot/score arrays](constrained.npz) and [run provenance](provenance.json)
