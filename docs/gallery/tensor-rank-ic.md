# Tensor rank selection: IC: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods tensor-rank-ic --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "tensor-rank-ic")
...
result = select_tensor_rank(x[:273], max_ranks=(2, 2), criterion='ic', penalty=2)
```
...
have no substituted estimate. [Full diagnostics](tensor-rank-ic.json) include
...
![Tensor rank selection: IC: computed output and diagnostics; see adjacent interpretation and JSON data](tensor-rank-ic.png)
...
[Numerical plot/score arrays](tensor-rank-ic.npz) and [run provenance](provenance.json)
