# Matrix GARCH: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#fit_matrix_garch)
...
Paper attribution: [yu2024garch](../citations.md#yu2024garch). This is a new teaching application,
...
```bash
python -m examples.air_quality --methods matrix-garch --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "matrix-garch")
...
have no substituted estimate. [Full diagnostics](matrix-garch.json) include
...
![Matrix GARCH: computed output and diagnostics; see adjacent interpretation and JSON data](matrix-garch.png)
...
[Numerical plot/score arrays](matrix-garch.npz) and [run provenance](provenance.json)
