# TOPUP tensor factors: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods topup --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "topup")
...
result = fit_tensor_factor(x[:273], ranks=(1, 1, 1), method='topup', iterative=False)
```
...
have no substituted estimate. [Full diagnostics](topup.json) include
...
Computed reconstruction_discrepancy: **0.5875** over 2079 observed target cells. 

![TOPUP tensor factors: computed output and diagnostics; see adjacent interpretation and JSON data](topup.png)
...
[Numerical plot/score arrays](topup.npz) and [run provenance](provenance.json)
