# Reduced-rank MAR: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods mar-reduced-rank --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "mar-reduced-rank")
...
result = fit_mar(x[:273], ranks=(1, 1), max_iter=500)
```
...
have no substituted estimate. [Full diagnostics](mar-reduced-rank.json) include
...
Computed forecast_mse: **1.3756** over 1042 observed target cells. Previous-day MSE: 1.8212; fixed-training-mean MSE: 1.6649. Lower is better on this split only; no uncertainty interval is estimated.

![Reduced-rank MAR: computed output and diagnostics; see adjacent interpretation and JSON data](mar-reduced-rank.png)
...
[Numerical plot/score arrays](mar-reduced-rank.npz) and [run provenance](provenance.json)
