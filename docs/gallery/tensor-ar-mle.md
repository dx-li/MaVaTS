# Tensor AR: MLE: an air-quality walkthrough
...
```bash
python -m examples.air_quality --methods tensor-ar-mle --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "tensor-ar-mle")
...
result = fit_tensor_ar(x[:273], method='mle', terms=(1,), max_iter=200)
```
...
have no substituted estimate. [Full diagnostics](tensor-ar-mle.json) include
...
Computed forecast_mse: **1.2068** over 2079 observed target cells. Previous-day MSE: 2.3205; fixed-training-mean MSE: 1.7401. Lower is better on this split only; no uncertainty interval is estimated.

![Tensor AR: MLE: computed output and diagnostics; see adjacent interpretation and JSON data](tensor-ar-mle.png)
...
[Numerical plot/score arrays](tensor-ar-mle.npz) and [run provenance](provenance.json)
