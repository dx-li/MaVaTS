# Matrix ARMA: MLE: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#fit_marma)
...
Paper attribution: [tsay2024marma](../citations.md#tsay2024marma). This is a new teaching application,
...
```bash
python -m examples.air_quality --methods marma-mle --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "marma-mle")
dates, values, counts = load()
x, observed, preprocessing = preprocess(values, counts, train=273, tensor=False)
from mavats import fit_marma
result = fit_marma(x[:273], method='mle', n_starts=1, max_iter=500)
```

## Computed result

Run status: **not_converged**. This records numerical execution, not
scientific validity. Unconverged output is diagnostic only; rejected fits
have no substituted estimate. [Full diagnostics](marma-mle.json) include
...
Computed forecast_mse: **1.3254** over 1042 observed target cells. Previous-day MSE: 1.8212; fixed-training-mean MSE: 1.6649. Lower is better on this split only; no uncertainty interval is estimated.

![Matrix ARMA: MLE: computed output and diagnostics; see adjacent interpretation and JSON data](marma-mle.png)
...
[Numerical plot/score arrays](marma-mle.npz) and [run provenance](provenance.json)
