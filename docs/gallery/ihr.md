# Entrywise Huber factors: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#fit_ihr_factor)
...
```bash
python -m examples.air_quality --methods ihr --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "ihr")
dates, values, counts = load()
x, observed, preprocessing = preprocess(values, counts, train=273, tensor=False)
from mavats import fit_ihr_factor
result = fit_ihr_factor(x[:273], ranks=(1, 1))
```
...
have no substituted estimate. [Full diagnostics](ihr.json) include
...
Computed reconstruction_discrepancy: **0.2619** over 1042 observed target cells. 

![Entrywise Huber factors: computed output and diagnostics; see adjacent interpretation and JSON data](ihr.png)
...
[Numerical plot/score arrays](ihr.npz) and [run provenance](provenance.json)
