# MAR rank selection: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#select_mar_rank)
...
Paper attribution: [xiao2022rrmar](../citations.md#xiao2022rrmar). This is a new teaching application,
...
```bash
python -m examples.air_quality --methods mar-rank-selection --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "mar-rank-selection")
dates, values, counts = load()
x, observed, preprocessing = preprocess(values, counts, train=273, tensor=False)
from mavats import select_mar_rank
result = select_mar_rank(x[:273], max_ranks=(2, 2), max_iter=500)
```
...
have no substituted estimate. [Full diagnostics](mar-rank-selection.json) include
...
Computed forecast_mse: **1.3606** over 1042 observed target cells. Previous-day MSE: 1.8212; fixed-training-mean MSE: 1.6649. Lower is better on this split only; no uncertainty interval is estimated.

![MAR rank selection: computed output and diagnostics; see adjacent interpretation and JSON data](mar-rank-selection.png)
...
[Numerical plot/score arrays](mar-rank-selection.npz) and [run provenance](provenance.json)
