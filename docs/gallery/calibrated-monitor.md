# Gaussian-reference monitor calibration: an air-quality walkthrough

[Gallery and data protocol](../real-world-examples.md) · [All examples](index.md) · [API](https://dx-li.github.io/MaVaTS/mavats.html#calibrate_monitor)
...
```bash
python -m examples.air_quality --methods calibrated-monitor --output /tmp/mavats-gallery
```
...
case = next(c for c in CASES if c.id == "calibrated-monitor")
...
have no substituted estimate. [Full diagnostics](calibrated-monitor.json) include
...
![Gaussian-reference monitor calibration: computed output and diagnostics; see adjacent interpretation and JSON data](calibrated-monitor.png)
...
[Numerical plot/score arrays](calibrated-monitor.npz) and [run provenance](provenance.json)
